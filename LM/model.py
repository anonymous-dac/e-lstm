...
class RNNModel(nn.Module):
    ...
    def forward(self, input, hidden, h_sparsity=0., h_threshold=0.):
        ...
        if self.rnn_type == "LSTM":
            ...
            for i in range(self.nlayers):
                if i == 0:
                    output, state = self.lstm_input(emb, hidden[0][i], hidden[1][i],h_sparsity=h_sparsity, h_threshold=h_threshold)
                else:
                    output, state = self.lstm_hidden(output, hidden[0][i], hidden[1][i], h_sparsity=h_sparsity, h_threshold=h_threshold)
                if i != self.nlayers - 1:
