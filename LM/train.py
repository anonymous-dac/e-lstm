...
parser.add_argument('--ws', type=float, default=0.4,
                    help='target sparsity for W')
parser.add_argument('--hs', type=float, default=0.6,
                    help='target sparsity for H')
parser.add_argument('--tied', action='store_true',
                    help='tie the word embedding and softmax weights')
parser.add_argument('--l1', action='store_true', 
                    help="Whether to add l1 regularization")
parser.add_argument('--l1_lambda', type=float, default=0.00001,
                    help="lambda of l1 regularization loss")
...
parser.add_argument('--save_dir', type=str, default='./LM/models/PTB/',
                    help='path to save the final model')
parser.add_argument('--log_dir', type=str, default='./LM/logs/PTB/',
                    help='path for log')
...
# Logging
###############################################################################
model_config_name = "model:{}-em:{}-nhid:{}-nlayers:{}-bptt:{}-epoch:{}-lr:{}-tied:{}-l1:{}-l1_lambda:{}-dropout:{}-ws:{}-hs:{}".format(
    args.model, args.emsize, args.nhid, args.nlayers, 
    args.bptt, args.epochs, args.lr, args.tied, args.l1, args.l1_lambda, args.dropout, args.ws, args.hs)
...
def pruning_mode(state_dict):
    for k, v in state_dict.items():
        if 'weight_x' in k:
            state_dict[k] = set_to_zero_sparsity(v, sparsity=args.ws)
        elif 'weight_h' in k:
            state_dict[k] = set_to_zero_sparsity(v, sparsity=args.hs)

logger.info('|Fine tuning start| Target W sparsity {:2f} |Target U sparsity {:2f}'.format(args.ws, args.hs))
fine_tune_epoch = int(args.epochs//4) + 1

# lr is a global variable
lr = get_lr(optimizer)
best_val_loss = None
try:
    ...
    logger.error('-' * 89)
    logger.error('Exiting from fine-tuning early')
