...
class LSTM(nn.Module):
    ...
    def forward(self, inputs, initial_h, initial_c, h_sparsity=0., h_threshold=0.):
        ...
        for t in range(time_steps):
            ...
            # Get sparse h
            if not h_f.requires_grad:
                if h_sparsity > 0.:
                    h_f = set_to_zero_sparsity(h_f, sparsity=h_sparsity)
                    if self.bidirectional:
                        h_b = set_to_zero_sparsity(h_b, sparsity=h_sparsity)
                elif h_threshold > 0.:
                    h_f = set_to_zero_threshold(h_f, threshold=h_threshold)
                    if self.bidirectional:
                        h_b = set_to_zero_threshold(h_b, threshold=h_threshold)
